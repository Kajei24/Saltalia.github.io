...
(() => {
  ...
  const loaderMinDuration = 1800; // SALTALIAのロゴを必ず最低1.8秒は見せる
  ...
  const scheduleFinishLoading = () => {
    const elapsed = performance.now() - loaderStartedAt;
    const delay = Math.max(0, loaderMinDuration - elapsed);
    window.setTimeout(finishLoading, delay);
  };
  if (loader) {
    if (document.readyState === 'complete') {
      scheduleFinishLoading();
    } else {
      window.addEventListener('load', scheduleFinishLoading, { once: true });
    }
  } else {
    document.body.classList.add('is-ready');
  }
  ...
  function createStars() {
    const count = Math.min(130, Math.floor((window.innerWidth * window.innerHeight) / 13000));
  ...
  function drawStars(time) {
    ...
    if (time - lastStarFrame < 1000 / STAR_FPS) { requestAnimationFrame(drawStars); return; }
    lastStarFrame = time;
  ...
  if (ctx && !prefersReducedMotion) {
    requestAnimationFrame(drawStars);
    window.setInterval(() => { if (!document.hidden && Math.random() < 0.55) spawnShootingStar(); }, 4500);
  } else if (ctx) {
    drawStars(0);
  }
